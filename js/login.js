...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (!loginForm) {
    console.error('🚨 로그인 폼을 찾을 수 없음! HTML의 ID가 loginForm인지 확인하세요.');
    return;
  }
  ...
  if (loginForm) {
    loginForm.addEventListener('submit', function (event) {
      ...
      if (!passwordValue) {
        if (passwordError) passwordError.textContent = '비밀번호를 입력해주세요.';
        console.log('비밀번호 누락');
        isValid = false;
      } else {
      ...
      if (isValid) {
        ...
        if (userIdValue === correctUserId && passwordValue === correctPassword) {
          ...
          if (passwordError) passwordError.textContent = '아이디 또는 비밀번호가 올바르지 않습니다.';
        }
      }
    });
